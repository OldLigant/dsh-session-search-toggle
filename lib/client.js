window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function initSwitchSearchState() {
			return {
		...
		function applySnapshot(draft, snap) {
			if (snap.revision !== void 0 && snap.revision <= draft.revision) return;
			const value = snap.value;
			if (value?.enabled !== void 0) draft.enabled = value.enabled;
			if (value?.defaultMode !== void 0) draft.defaultMode = value.defaultMode;
			if (snap.revision !== void 0) draft.revision = snap.revision;
		...
		const switchSearchStore = {
			spec: {
				init: initSwitchSearchState,
				actions: { sync: applySnapshot }
			},
			create: () => {
				...
				const sync = (snap) => {
					...
					if (next.revision === state.revision && next.enabled === state.enabled && next.defaultMode === state.defaultMode && next.writable === state.writable && next.unavailable === state.unavailable) return;
					state = next;
